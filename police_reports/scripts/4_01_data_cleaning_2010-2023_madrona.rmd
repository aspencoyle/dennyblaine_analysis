...
This script cleans and organizes police data on reports from Madrona Beach, with the goal of examining police response frequency over time
...
```{r}
# Read in data
madrona_call_dat <- read.csv("../data/db_call_data_2010-2023.csv")

# Woof, 237,000 rows! 

# Let's narrow it down some, and only select those calls with a blurred lat/long within a quarter mile of Madrona Beach
# North boundary: 47.612561
# South boundary: 47.6059142
# East boundary: -122.278256
# West boundary: -122.287389

# Filter by location
madrona_call_dat <- madrona_call_dat %>%
  dplyr::filter(Blurred_Latitude < 47.612561 & Blurred_Latitude > 47.6059142) %>%    # north/south
  dplyr::filter(Blurred_Longitude < -122.278256 & Blurred_Longitude > -122.287389)  # east/west 
...
madrona_call_dat$Final.Call.Type <- str_remove(madrona_call_dat$Final.Call.Type, "--")


# Look at different reasons calls were made
table(madrona_call_dat$Initial.Call.Type)
...
# Alright, that takes care of every incident type that had a DB match! Let's examine the dataframe again for incident types only present at Madrona Beach, and filter out if they're not potentially park-related
table(madrona_call_dat$Initial.Call.Type)

madrona_call_dat <- madrona_call_dat %>%
  filter(Initial.Call.Type != "ASSIST OTHER AGENCY - EMERGENCY SERVICE") %>%          # checked, all are vague assistance
  filter(Initial.Call.Type != "OBS - ACC - UNK INJURIES") %>%                         # checked, all are motor vehicle collisions
  filter(Initial.Call.Type != "PEACE-STANDBY TO ASSURE (NO COURT ORDR SVC)") %>%      # checked, all are DV or vague
  filter(Initial.Call.Type != "REQUEST TO WATCH")                                     # checked, all premise checks
...
table(madrona_call_dat$Final.Call.Type)

# Remove ones that can't be Madrona Park-related. Again, we'll start with the types seen at Denny-Blaine that can't be park-related
madrona_call_dat <- madrona_call_dat %>%
...
# This catches any types seen here and not at DB or Madison Beach

table(madrona_call_dat$Final.Call.Type)

# Nothing sticks out as odd by itself!

# Let's also do a quick check of event resolution

table(madrona_call_dat$Event.Clearance.Description)
...
# Manual check for combinations we can remove from Madrona Beach
madrona_call_dat <- madrona_call_dat %>%
...
Cool, we've got a list of incidents that could have potentially been at Madrona Beach
...
madrona_call_dat$broad_category <- NA
...
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ALARM - DURESS/PANIC,TAXI/CAR/PRSN - NOT DV"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ANIMAL - INJURED, DEAD HAZARD, OTHER"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASLT - CRITICAL (NO SHOOTINGS)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASLT - REPORT FOR PERSON SHOT OR SHOT AT"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASLT - WITH OR W/O WPNS (NO SHOOTINGS)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "AUTO - THEFT/LOSS OF PLATES"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "AUTO THEFT - VEH THEFT OR THEFT AND RECOVERY"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "BIAS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "BURN - RECKLESS BURNING"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "DISTURBANCE"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "EXPLOSION"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "FIGHT - CRITICAL WITH WEAPONS"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "FIGHT - PHYSICAL (NO WEAPONS)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "FIREWORKS (NO HAZARD)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HARAS - NO BIAS OR THREATS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ILLEGAL DUMPING"] <- "prop.dmg"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "LEWD -  (EXPOSING, FLASHING)"] <- "lewd"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "LIQUOR VIOLATIONS - ADULT"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "LIQUOR VIOLATIONS - MINOR"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "NARCOTICS - VIOLATIONS (LOITER, USE, SELL, NARS)"] <- "drug"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "NOISE - ANIMAL (INCLUDES BARKING DOGS)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "NOISE - GENERAL"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "NUISANCE"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - ANIMAL - IP/JO - BITE"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - ARSON - IP/JO"] <- "prop.dmg"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - EXPLOSION - IP/JO"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - FIGHT - IP - PHYSICAL (NO WEAPONS)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - FIGHT - VERBAL/ORAL (NO WEAPONS)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - MENTAL - PERSON OR PICKUP/TRANSPORT"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - NOISE - DISTURBANCE (PARTY, ETC)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - PROWLER"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - PROWLER - IP/JO"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - SECONDARY - PROPERTY DAMAGE/DESTRUCTION"] <- "prop.dmg"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - SECONDARY - THEFT"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS WEAPN-IP/JO-GUN,DEADLY WPN (NO THRT/ASLT/DIST)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OVERDOSE"] <- "drug"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PARKING VIOLATION (EXCEPT ABANDONED CAR)"] <- "traffic"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PARKS VIOLATIONS/CITES (INCLUDES EXCLUSIONS)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PERSON IN BEHAVIORAL/EMOTIONAL CRISIS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PREMISE CHECK, OFFICER INITIATED ONVIEW ONLY"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PROPERTY - DAMAGE"] <- "prop.dmg"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PROPERTY - DAMAGE (GUN RELATED)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ROBBERY - CRITICAL (INCLUDES STRONG ARM)"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ROBBERY (INCL STRONG ARM)"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "SEX IN PUBLIC PLACE (INCL MASTURBATION)"] <- "lewd"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "SHOTS HEARD - DELAY"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "SHOTS HEARD - IN PROGRESS"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "SUSPICIOUS PERSON, VEHICLE, OR INCIDENT"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "SUSPICIOUS STOP - OFFICER INITIATED ONVIEW"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "THEFT (DOES NOT INCLUDE SHOPLIFT OR SVCS)"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "THREATS (INCLS IN-PERSON/BY PHONE/IN WRITING)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRAFFIC - BLOCKING ROADWAY"] <- "traffic"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRESPASS"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - ASLT - MOLESTED ADULT (GROPED, FONDLED, ETC)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - DISTURBANCE"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - HARASSMENT"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - PROPERTY DESTRUCTION/DAMAGE"] <- "prop.dmg"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - SEX IN PUBLIC"] <- "lewd"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - SUSPICIOUS CIRCUMSTANCES"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - THEFT"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "UNKNOWN - ANI/ALI - LANDLINE (INCLUDES OPEN LINE)"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "UNKNOWN - COMPLAINT OF UNKNOWN NATURE"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "WEAPN - GUN,DEADLY WPN (NO THRTS/ASLT/DIST)"] <- "violence"

# Doing the same for ones that were seen at Madison Beach, but not DB

madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ALARM - AUDIBLE AUTOMOBILE (UNOCC/ANTI-THEFT)"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ALARM - DURESS/PANIC,BUS/TAXI/CAR/PRSN - NOT DV"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ANIMAL - DANGEROUS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ANIMAL BITE"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ARSON - REPORT"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASLT - MOLESTED ADULT (GROPED, FONDLED, ETC.)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASLT - PERSON SHOT OR SHOT AT"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "LITTERING"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - ANIMAL - IP/JO - DANGEROUS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - AUTO THEFT-IP/JO - VEHICLE, PLATES, TABS"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - DOA - CASUALTY, DEAD BODY"] <- "violence"      # homicide
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - PURSE SNATCH - IP/JO - ROBBERY"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "OBS - UNKNOWN - ANI/ALI - PAY PHN (INCL OPEN LINE)"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "PROPERTY - LOST OR MISSING"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "RAPE"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - ASLT - WITH OR W/O WPNS (NO SHOOTINGS)"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - AUTO THEFT"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - LEWD CONDUCT"] <- "lewd"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - PROPERTY, LOST OR MISSING"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "TRU - THREATS"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "URINATING, DEFECATING IN PUBLIC"] <- "lewd"

# Looks like we only have one new one that can be easily defined just by looking at Initial Call Type.
# Let's assign that now
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "DETOX - PICKUP (FIRE / POLICE STANDING BY)"] <- "drug"
...
# Let's sort those now, using our assignments from DB and Madison Beach
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASSIST OTHER AGENCY - EMERGENCY SERVICE" &
                                  madrona_call_dat$Final.Call.Type == "AUTOMOBILES - AUTO THEFT"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "ASSIST OTHER AGENCY - EMERGENCY SERVICE" &
                                  madrona_call_dat$Final.Call.Type == "DISTURBANCE - OTHER"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "ARSON,BOMBS,EXPLO - BOMBS, EXPLO,LARGE FIREWORKS"] <- "violence"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "DISTURBANCE - OTHER"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "MISCHIEF OR NUISANCE - GENERAL"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "PROWLER - TRESPASS"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "SUSPICIOUS CIRCUM. - SUSPICIOUS PERSON"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "THEFT - CAR PROWL"] <- "theft"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "UNKNOWN - ANI/ALI - WRLS PHNS (INCL OPEN LINE)" &
                                  madrona_call_dat$Final.Call.Type == "SUSPICIOUS CIRCUM. - SUSPICIOUS PERSON"] <- "suspicious"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "UNKNOWN - ANI/ALI - WRLS PHNS (INCL OPEN LINE)" &
                                  madrona_call_dat$Final.Call.Type == "DISTURBANCE - OTHER"] <- "disturbance"

# See if we have any remaining NAs
table(madrona_call_dat$broad_category, exclude = NULL)

# Just a few left! Let's go through these individually
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "HAZ - POTENTIAL THRT TO PHYS SAFETY" &
                                  madrona_call_dat$Final.Call.Type == "CRISIS COMPLAINT - GENERAL"] <- "disturbance"
madrona_call_dat$broad_category[madrona_call_dat$Initial.Call.Type == "UNKNOWN - ANI/ALI - WRLS PHNS (INCL OPEN LINE)" &
                                  madrona_call_dat$Final.Call.Type == "MISCHIEF OR NUISANCE - GENERAL"] <- "disturbance"
...
madrona_call_dat$Original.Time.Queued <- str_split_i(madrona_call_dat$Original.Time.Queued, " ", 1)
...
# Check column type
str(madrona_call_dat$Incident.Date)
# Yep it's a character string

# Transform into Date object type
madrona_call_dat$Incident.Date <- mdy(madrona_call_dat$Incident.Date)
# Check it worked
str(madrona_call_dat$Incident.Date)

# Write everything out
write.csv(madrona_call_dat, file = "../output/potential_madrona_calls_2010-2023.csv", row.names = FALSE)
```
